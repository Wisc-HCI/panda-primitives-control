        ...
        while (not self._action_status and not rospy.is_shutdown()):
            ...
            if self.a_server.is_preempt_requested():
                ...
                if self._screw_count == 0:
                    ...
                    #Convert GOAL_POSE to cf
                    if (self._screw_current_pose[3]!=0 and self._screw_current_pose[4]!=0 and self._screw_current_pose[5]!=0 and 
                    self._screw_current_pose[6]!=1):
                        ...
                        #Conversion
                        self._goal_pose[0] = r11*self._goal_pose[0] + r21*self._goal_pose[1] + r31*self._goal_pose[2]
                        self._goal_pose[1] = r12*self._goal_pose[0] + r22*self._goal_pose[1] + r32*self._goal_pose[2]
                        self._goal_pose[2] = r13*self._goal_pose[0] + r23*self._goal_pose[1] + r33*self._goal_pose[2]

                        self._goal_pose[3] = self._goal_pose[6]*icqw - self._goal_pose[3]*icqx - self._goal_pose[4]*icqy - self._goal_pose[5]*icqz
                        self._goal_pose[4] = self._goal_pose[6]*icqx + self._goal_pose[3]*icqw - self._goal_pose[4]*icqz + self._goal_pose[5]*icqy
                        self._goal_pose[5] = self._goal_pose[6]*icqy + self._goal_pose[3]*icqz + self._goal_pose[4]*icqw - self._goal_pose[5]*icqx
                        self._goal_pose[6] = self._goal_pose[6]*icqz - self._goal_pose[3]*icqy + self._goal_pose[4]*icqx + self._goal_pose[5]*icqw
                ...
                if self._screw_count == 0:
                    ...
                    #Convert GOAL_POSE to cf
                    if (self._init_goal_pose[3]!=0 and self._init_goal_pose[4]!=0 and self._init_goal_pose[5]!=0 and 
                    self._init_goal_pose[6]!=1):
...
#####################################################################################################
    ...
    def primitive_stop_force(self):
        ...
        #CONVERT CURRENT_POSE INTO CF
        if (self._goal_action == 'rotate screw' or self._goal_action == 'rotate_back'):

            if (self._screw_current_pose[3]!=0 and self._screw_current_pose[4]!=0 and self._screw_current_pose[5]!=0 and 
            self._screw_current_pose[6]!=1):

                constraint_frame = [current_pose[3], current_pose[4], current_pose[5], current_pose[6]]
                ...
                #Conversion
                current_pose[0] = r11*current_pose[0] + r21*current_pose[1] + r31*current_pose[2]
                current_pose[1] = r12*current_pose[0] + r22*current_pose[1] + r32*current_pose[2]
                current_pose[2] = r13*current_pose[0] + r23*current_pose[1] + r33*current_pose[2]

                current_pose[3] = current_pose[6]*icqw - current_pose[3]*icqx - current_pose[4]*icqy - current_pose[5]*icqz
                current_pose[4] = current_pose[6]*icqx + current_pose[3]*icqw - current_pose[4]*icqz + current_pose[5]*icqy
                current_pose[5] = current_pose[6]*icqy + current_pose[3]*icqz + current_pose[4]*icqw - current_pose[5]*icqx
                current_pose[6] = current_pose[6]*icqz - current_pose[3]*icqy + current_pose[4]*icqx + current_pose[5]*icqw
